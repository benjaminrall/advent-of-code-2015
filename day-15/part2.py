...
def search(ingredients, calories, choices = [], total=100, i=0):
    if i + 1 == len(ingredients):
        choices += [100 - sum(choices)]
        if sum([c1 * c2 for c1, c2, in zip(choices, calories)]) != 500:
            return 0
        return np.prod(np.maximum(np.sum([ingredient * c for ingredient, c in zip(ingredients, choices)], axis=0), 0))
    
    best = -np.inf
    for n in range(total + 1):
        best = max(best, search(ingredients, calories, choices + [n], total - n, i + 1))
...
def solve(filename: str) -> int:
    ...
    return search(ingredients, calories)
